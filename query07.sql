...
SELECT Drug.name, 
       ...
       CASE WHEN DnD.price = MNXP.mxPrice AND DnD.price = MNXP.mnPrice THEN 'min=max'
        ELSE CASE WHEN DnD.price = MNXP.mxPrice THEN 'max'
                  WHEN DnD.price = MNXP.mnPrice THEN 'min' END END AS indicator
    FROM Drug 
        ...
        JOIN MinMaxPrice AS MNXP ON 
            Drug.name = MNXP.name;-- AND (price = mxPrice OR price = mnPrice);
